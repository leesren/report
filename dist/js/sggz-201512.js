...
(function(window) {
    ...
    function loadData(dateType, path, map, segment) {
	        ...
	        url = path + 'api/doSta/staExtrawage';
    ...
    */
    ...
    function showSale(arr) {
        ...
        var html = _.template($('#sggz-tpl').html());
        var dom = html({
            data: arr
        });
        render(dom);
    ...
    */
	...
	function saveDataTolocal(target) {
                    var id = $(target).attr('data-store');
                    if (id) {
                        toggle(target,$(target).parent().find('tr'));
                        window.setTimeout(function() {
                            window.location.href = 'page6.html?storeId=' + storeId + '&type=1&eid=' + (id);
                        }, 600);
                    }
	}
    /**
